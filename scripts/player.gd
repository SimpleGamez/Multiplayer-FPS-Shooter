	...
	if Input.is_action_just_pressed("shoot") \
		...
		if raycast.is_colliding() && str(raycast.get_collider()).contains("CharacterBody3D") :
			...
			if hit_player.is_in_group("player"):
				var killer_id = get_multiplayer_authority()
				hit_player.killer = self
				hit_player.killer_obj = self.get_multiplayer_authority()
				hit_player.recieve_damage.rpc_id(hit_player.get_multiplayer_authority(), 1, killer_id)
...
func _physics_process(delta: float) -> void:
	...
	$Camera3D/Label.text = "KILL : " + str(Global.kill_count)
...
@rpc("any_peer")
func recieve_damage(damage := 1, killer_id := -1) -> void:
	health -= damage
	if health <= 0:
		health = 2
		if killer_id != -1:
			killer_obj = killer_id
			killer = get_tree().get_multiplayer().get_remote_sender_id() # optional if you want actual peer
			add_kill.rpc_id(killer_obj)
		position = spawns[randi() % spawns.size()]
